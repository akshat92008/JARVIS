...
class TestAmauraGrowthSystem(unittest.TestCase):
    def setUp(self):
        ...
        self.pipeline.create_campaign(
            campaign_id="agency14", name="14-day agency partner", target_segment="Small agencies",
            offer="White-label product engineering", daily_lead_limit=100, daily_outreach_limit=3,
        )
    ...
    def _qualified_lead(self, number: int = 1):
        lead = self.pipeline.discover_lead(
            campaign_id="agency14", company_name=f"Example Agency {number}",
            domain=f"agency-{number}.example.com", source_url=f"https://agency-{number}.example.com/services",
        )
        self.pipeline.transition(lead["id"], "researching", actor="prospect_research", reason="Research started")
        self.pipeline.add_evidence(
            lead["id"], claim_type="services", claim="Lists marketing services",
            source_url=f"https://agency-{number}.example.com/services",
            source_excerpt="The agency publicly lists branding, SEO and paid advertising services.", confidence=0.9,
        )
        self.pipeline.transition(lead["id"], "researched", actor="prospect_research", reason="Evidence complete")
        return self.pipeline.score_lead(lead["id"], {
    ...
    def test_new_workflows_build_complete_dependency_graphs(self):
        revenue = self.control.create_program(
            objective="Secure one ethical paid agency trial", success_metric="One founder-approved trial",
        ...
        content = self.control.create_program(
            objective="Publish one verified proof package", success_metric="Every claim and asset passes QA",
    ...
    def test_opt_out_and_kill_switch_are_immediate(self):
        ...
        with self.assertRaisesRegex(GovernanceError, "kill switch"):
            self.pipeline.discover_lead(
                campaign_id="agency14", company_name="Blocked", domain="blocked.example.com",
    ...
    def test_content_factory_requires_hashes_licences_and_complete_qa_set(self):
        ...
        factory.create_campaign(
            campaign_id="content1", title="Proof of work", audience="Founders",
        ...
        with self.assertRaisesRegex(GovernanceError, "source URL and recorded licence"):
            factory.register_asset(
                "content1", asset_type="stock", uri="https://cdn.example.com/clip.mp4", sha256="a" * 64,
            )
        for index, asset_type in enumerate(sorted(REQUIRED_PUBLICATION_ASSETS)):
            factory.register_asset(
                "content1", asset_type=asset_type, uri=f"artifact://content1/{asset_type}",
                sha256=f"{index:064x}", status="approved",
            )
        self.assertTrue(factory.publication_readiness("content1")["ready"])
        first = factory.record_metrics("content1", platform="youtube", window="24h", metrics={"views": 10, "qualified_enquiries": 1})
        second = factory.record_metrics("content1", platform="youtube", window="24h", metrics={"views": 12, "qualified_enquiries": 1})
        self.assertEqual(first["id"], second["id"])

    def test_concurrent_domain_deduplication_and_integrity(self):
        def discover(_: int):
            return self.pipeline.discover_lead(
                campaign_id="agency14", company_name="Concurrent Agency", domain="www.concurrent.example.com",
    ...
    def test_concurrent_daily_discovery_cap_is_atomic(self):
        self.pipeline.create_campaign(
            campaign_id="tiny", name="Tiny campaign", target_segment="Agencies",
            offer="Trial", daily_lead_limit=5,
        )

        def discover(number: int) -> bool:
            try:
                self.pipeline.discover_lead(
                    campaign_id="tiny", company_name=f"Tiny {number}", domain=f"tiny-{number}.example.com",
