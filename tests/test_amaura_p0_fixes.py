...
class TestEvidenceRoundTrip:
    def test_get_text_via_canonical_reference(self, tmp_path):
        ...
        assert ref.startswith("evidence://sha256/"), f"unexpected ref format: {ref}"
    ...
    def test_verify_accepts_canonical_reference(self, tmp_path):
        """verify() must work with the canonical evidence://sha256/ reference."""
        vault = _vault(tmp_path)
...
class TestReviewerKeyResolution:
    ...
    def test_matches_correct_key(self):
        env = {"AMAURA_REVIEWER_KEYS": "qa_agent:abc123,senior_qa:xyz789"}
        with patch.dict(os.environ, env):
            from jarvis.server import _resolve_reviewer_from_key
            assert _resolve_reviewer_from_key("abc123") == "qa_agent"
            assert _resolve_reviewer_from_key("xyz789") == "senior_qa"

    def test_rejects_wrong_key(self):
        env = {"AMAURA_REVIEWER_KEYS": "qa_agent:abc123"}
        with patch.dict(os.environ, env):
            from jarvis.server import _resolve_reviewer_from_key
