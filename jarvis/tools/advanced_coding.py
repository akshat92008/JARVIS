...
def tool_install_dependencies(packages: list, manager: str = "", dev: bool = False, cwd: str = "") -> str:
    ...
    commands = {
        ...
        "pip3": f"pip3 install {' '.join(packages)}",
        "npm": f"npm install {'--save-dev' if dev else ''} {' '.join(packages)}",
        "yarn": f"yarn add {'--dev' if dev else ''} {' '.join(packages)}",
        "pnpm": f"pnpm add {'--save-dev' if dev else ''} {' '.join(packages)}",
        "cargo": f"cargo add {' '.join(packages)}",
        "go": f"go get {' '.join(packages)}",
        "brew": f"brew install {' '.join(packages)}",
        "composer": f"composer require {'--dev' if dev else ''} {' '.join(packages)}",
    }
...
def tool_run_tests(path: str = ".", framework: str = "", filter: str = "", verbose: bool = True, cwd: str = "") -> str:
    ...
    commands = {
        "pytest": f"python -m pytest {path} {'-v' if verbose else ''} {f'-k {filter}' if filter else ''}",
        "unittest": f"python -m unittest discover {path} {'-v' if verbose else ''}",
        "jest": f"npx jest {path} {'--verbose' if verbose else ''} {f'--testNamePattern={filter}' if filter else ''}",
        "vitest": f"npx vitest run {path}",
        "mocha": f"npx mocha {path}",
        "go": f"go test {'./...' if path == '.' else path} {'-v' if verbose else ''} {f'-run {filter}' if filter else ''}",
        "cargo": f"cargo test {filter or ''}",
        "rspec": f"bundle exec rspec {path}",
        "phpunit": f"./vendor/bin/phpunit {path}",
    }
    ...
    try:
        result = subprocess.run(cmd.strip(), shell=True, cwd=work_dir, capture_output=True, text=True, timeout=300)
...
def tool_lint_code(path: str, fix: bool = False, linter: str = "", cwd: str = "") -> str:
    ...
    commands = {
        "ruff": f"python -m ruff check {path} {'--fix' if fix else ''}",
        "flake8": f"python -m flake8 {path}",
        "eslint": f"npx eslint {path} {'--fix' if fix else ''}",
        "golangci-lint": f"golangci-lint run {path}",
        "clippy": "cargo clippy",
    }
    ...
    try:
        result = subprocess.run(cmd.strip(), shell=True, cwd=work_dir, capture_output=True, text=True, timeout=60)
        output = (result.stdout + "\n" + result.stderr).strip()
        if result.returncode == 0 and not output:
            return f"✅ No lint issues found ({linter})."
        return f"Lint results ({linter}):\n{output[:3000]}"
    except Exception as e:
        return f"❌ Lint error: {e}. Is {linter} installed?"
...
def tool_format_code(path: str, formatter: str = "", check_only: bool = False, cwd: str = "") -> str:
    ...
    commands = {
        "black": f"python -m black {path} {'--check' if check_only else ''}",
        "ruff": f"python -m ruff format {path} {'--check' if check_only else ''}",
        "prettier": f"npx prettier {'--check' if check_only else '--write'} {path}",
        "gofmt": f"gofmt {'-l' if check_only else '-w'} {path}",
        "rustfmt": f"rustfmt {path} {'--check' if check_only else ''}",
        "clang-format": f"clang-format {'-n' if check_only else '-i'} {path}",
    }
    ...
    try:
        result = subprocess.run(cmd.strip(), shell=True, cwd=work_dir, capture_output=True, text=True, timeout=60)
        if result.returncode == 0:
            return f"✅ {'Check passed' if check_only else 'Formatted'} with {formatter}: {path}"
        return f"Format result ({formatter}):\n{(result.stdout + result.stderr).strip()[:2000]}"
    except Exception as e:
        return f"❌ Format error: {e}. Is {formatter} installed?"
...
def tool_create_tests(source_path: str, output_path: str = "", framework: str = "", style: str = "unit") -> str:
    ...
    if not framework:
        if lang == "python": framework = "pytest"
        elif lang in ("javascript", "typescript"): framework = "jest"
        elif lang == "go": framework = "go"
        elif lang == "rust": framework = "cargo"
    ...
    if not output_path:
        if framework in ("pytest", "unittest"):
            ...
            output_path = str(p.parent / f"{p.stem}_test.go")
        else:
            output_path = str(p.parent / f"test_{p.name}")
...
def _generate_python_tests(content: str, path: Path, framework: str) -> str:
    ...
    lines = [f'"""Tests for {path.name}."""', f"from {module_name} import *", ""]
...
def _generate_js_tests(content: str, path: Path, framework: str) -> str:
    """Generate JS test skeletons."""
    functions = re.findall(r'(?:export\s+)?(?:async\s+)?function\s+(\w+)', content)
    arrow_fns = re.findall(r'(?:export\s+)?(?:const|let)\s+(\w+)\s*=\s*(?:async\s+)?\(', content)
    all_fns = functions + arrow_fns

    lines = [f'// Tests for {path.name}', ""]
    module = f"./{path.stem}"

    if all_fns:
        lines.append(f"const {{ {', '.join(all_fns)} }} = require('{module}');\n")
...
def tool_manage_env(action: str, env_type: str = "python", name: str = ".venv", python_version: str = "", cwd: str = "") -> str:
    ...
    if env_type == "python":
        env_path = Path(work_dir) / name

        if action == "create":
            python_cmd = f"python{python_version}" if python_version else "python3"
            try:
                result = subprocess.run(
                    f"{python_cmd} -m venv {name}",
                    shell=True, cwd=work_dir, capture_output=True, text=True, timeout=30,
                )
                if result.returncode != 0:
            ...
            if req_file.exists():
                ...
                if not pip_path.exists():
                    pip_path = Path(work_dir) / name / "bin" / "pip3"
                try:
                    result = subprocess.run(
                        f"{pip_path} install -r requirements.txt",
                        shell=True, cwd=work_dir, capture_output=True, text=True, timeout=120,
                    )
                    if result.returncode == 0:
                        return f"✅ Dependencies installed in {name}"
            ...
            return f"❌ Environment not found: {name}"

    return f"❌ Unsupported action '{action}' for env_type '{env_type}'."
