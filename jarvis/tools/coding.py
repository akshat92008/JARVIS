...
def tool_run_command(command: str, cwd: str | None = None, timeout: int = 120) -> str:
    ...
    try:
        ...
        return f"❌ Cannot execute command: {e}"
...
def tool_git_status(cwd: str | None = None) -> str:
    ...
    try:
        branch = subprocess.run(
            "git branch --show-current", shell=True, cwd=work_dir,
            capture_output=True, text=True, timeout=10,
        )
        status = subprocess.run(
            "git status --short", shell=True, cwd=work_dir,
            capture_output=True, text=True, timeout=10,
        )
...
def tool_git_commit(message: str, files: list | None = None, all: bool = False, cwd: str | None = None) -> str:
    ...
    try:
        if all:
            subprocess.run("git add -A", shell=True, cwd=work_dir, timeout=10)
        elif files:
            for f in files:
                subprocess.run(f"git add {f}", shell=True, cwd=work_dir, timeout=10)

        result = subprocess.run(
            f'git commit -m "{message}"', shell=True, cwd=work_dir,
            capture_output=True, text=True, timeout=30,
        )
        if result.returncode != 0:
...
def tool_git_log(count: int = 10, oneline: bool = True, cwd: str | None = None) -> str:
    ...
    try:
        result = subprocess.run(
            f"git log -n {count}{fmt}", shell=True, cwd=work_dir,
            capture_output=True, text=True, timeout=10,
            env={**os.environ, "GIT_PAGER": "cat"},
        )
