...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function addToolEventBadge(toolName, args, completed = false) {
        ...
        badge.innerHTML = `
            ...
            <span>${completed ? 'Tool Completed:' : 'Executing Tool:'} <strong>${toolName}</strong></span>
        `;
    ...
    function updateToolEventBadge(toolName) {
        ...
        if (badges.length > 0) {
            ...
            lastBadge.querySelector("span").innerHTML = `Tool Completed: <strong>${toolName}</strong>`;
        }
    }
    ...
    async function fetchAmauraDashboard() {
        ...
        try {
            const dashboardResponse = await fetch("/api/amaura/dashboard");
    ...
    function renderToolsList(tools) {
        ...
        tools.forEach((t) => {
            ...
            item.innerHTML = `
                <div class="tool-name"><i class="fa-solid fa-wrench"></i> ${t.name}</div>
    ...
    function renderMemoryList(facts) {
        ...
        facts.forEach((fact) => {
            ...
            item.innerHTML = `
                <span><i class="fa-solid fa-lightbulb"></i> ${fact}</span>
            `;
